...
import static org.eclipse.jdt.internal.core.index.solr.indexer.IndexLog.createStatus;
...
import org.eclipse.jdt.internal.core.index.solr.java.SolrAnnotation;
import org.eclipse.jdt.internal.core.index.solr.java.SolrConstant;
import org.eclipse.jdt.internal.core.index.solr.java.SolrConstantAnnotation;
import org.eclipse.jdt.internal.core.index.solr.java.SolrConstantArray;
import org.eclipse.jdt.internal.core.index.solr.java.SolrConstantClass;
import org.eclipse.jdt.internal.core.index.solr.java.SolrConstantEnum;
...
import org.eclipse.jdt.internal.core.index.solr.java.SolrTypeAnnotation;
...
public final class ClassFileToIndexConverter {
	...
	private final SolrResourceFile resource;
	private final SolrIndex index;

	public ClassFileToIndexConverter(SolrResourceFile resourceFile, SolrIndex index) {
	...
	protected static IBinaryType createInfoFromClassFileInJar(Openable classFile) throws CoreException {
		...
		try {
			...
			throw new CoreException(createStatus("Unable to parse JAR file", e)); //$NON-NLS-1$
		} finally {
	 ...
	 * 
	 * @param binaryType
	 *            an object used for parsing the .class file itself
	 * @param fieldDescriptor
	 *            the name that is used to locate the class, computed from the .class file's name and location. In the
	 *            event that the .class file has been moved, this may differ from the binary name stored in the .class
	 *            file itself, which is why this is received as an argument rather than extracted from the .class file.
	 * @throws CoreException
	 */
	public SolrType addType(IBinaryType binaryType, char[] fieldDescriptor, IProgressMonitor monitor)
			throws CoreException {
		...
		String name = new String(fieldDescriptor);
		SolrType type = this.index.findType(name, this.resource);

		if (type == null) {
			type = new SolrType(this.resource);
			type.setName(name);
		}
		...
		if (typeAnnotations != null) {
			for (IBinaryTypeAnnotation typeAnnotation : typeAnnotations) {
				SolrTypeAnnotation annotation = new SolrTypeAnnotation(type);
	...
	private void addMethod(NdMethod method, IBinaryMethod next, IBinaryType binaryType) throws CoreException {
		...
		if (compilerDefinedParametersAreIncludedInSignature) {
			...
			if (binaryType.isMember() && next.isConstructor() && ((binaryType.getModifiers() & Modifier.STATIC) == 0)
		...
		int estimatedParameterCount = Math.max(
				Math.max(Math.max(numArgumentsInGenericSignature, namedParameterCount), annotatedParametersCount),
				parameterFieldDescriptors.size());
	...
	private NdTypeSignature parseClassTypeSignature(NdComplexTypeSignature parentTypeOrNull, SignatureWrapper wrapper)
		...
		if (parentTypeOrNull != null) {
			fieldDescriptor = CharArrayUtils.concat(
					parentTypeOrNull.getRawType().getFieldDescriptorWithoutTrailingSemicolon(), INNER_TYPE_SEPARATOR,
					identifier, FIELD_DESCRIPTOR_SUFFIX);
		} else {
	...
	private void initTypeAnnotation(SolrTypeAnnotation annotation, IBinaryTypeAnnotation next) {
		annotation.setTypePath(next.getTypePath());
		int targetType = next.getTargetType();
		annotation.setTargetType(targetType);
		switch (targetType) {
			case AnnotationTargetTypeConstants.METHOD_TYPE_PARAMETER:
			case AnnotationTargetTypeConstants.CLASS_TYPE_PARAMETER:
				annotation.setTypeParameterIndex(next.getTypeParameterIndex());
				break;
			case AnnotationTargetTypeConstants.CLASS_EXTENDS:
				annotation.setSupertypeInde(next.getSupertypeIndex());
				break;
			case AnnotationTargetTypeConstants.CLASS_TYPE_PARAMETER_BOUND:
			case AnnotationTargetTypeConstants.METHOD_TYPE_PARAMETER_BOUND:
				annotation.setTypeParameterIndex(next.getTypeParameterIndex());
				annotation.setBoundIndex(next.getBoundIndex());
				break;
			...
			case AnnotationTargetTypeConstants.METHOD_FORMAL_PARAMETER:
				annotation.setMethodFormalParameterIndex(next.getMethodFormalParameterIndex());
				break;
			case AnnotationTargetTypeConstants.THROWS:
				annotation.setThrowsTypeIndex(next.getThrowsTypeIndex());
				break;
	...
	private void initAnnotation(SolrAnnotation annotation, IBinaryAnnotation next) {
		annotation.setTypeName(createTypeIdFromBinaryName(next.getTypeName()));
		...
		if (pairs != null) {
			...
			for (IBinaryElementValuePair element : pairs) {
				annotation.NdAnnotationValuePair nextPair = annotation.createValuePair(element.getName());
	...
	private String createTypeIdFromBinaryName(char[] binaryName) {
		if (binaryName == null) {
			return null;
		}

		return new String(JavaNames.binaryNameToFieldDescriptor(binaryName));
	}
	 ...
	 */
	public SolrConstant createConstantFromMixedType(Object value) {
		if (value instanceof Constant) {
			Constant constant = (Constant) value;

			return SolrConstant.create(constant);
			...
			return new SolrConstantClass(new String(binaryName));
		} else if (value instanceof IBinaryAnnotation) {
			IBinaryAnnotation binaryAnnotation = (IBinaryAnnotation) value;

			SolrAnnotation annotation = new SolrAnnotation();
			SolrConstantAnnotation constant = new SolrConstantAnnotation(annotation);
			initAnnotation(constant.getValue(), binaryAnnotation);
			return constant;
		} else if (value instanceof Object[]) {
			SolrConstantArray result = new SolrConstantArray();
			Object[] array = (Object[]) value;
			result.setValue(Arrays.stream(array).map(this::createConstantFromMixedType).collect(Collectors.toList()));
			return result;
		} else if (value instanceof EnumConstantSignature) {
			EnumConstantSignature signature = (EnumConstantSignature) value;
			
			SolrConstantEnum result = new SolrConstantEnum(createTypeIdFromBinaryName(signature.getTypeName()),
